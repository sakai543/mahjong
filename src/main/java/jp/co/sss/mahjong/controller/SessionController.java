...
public class SessionController {
	...
	@RequestMapping(path = "/doWhich2", method = RequestMethod.GET)
	public String result2(String win, String lose) {
		return "result2";
	}
	...
	public String pinhu(pinhuForm p, ModelMap model) {
		...
		System.out.println(p.isRiyu1());
		System.out.println(p.isRiyu2());
		System.out.println(p.isRiyu3());

		if (p.isRiyu1() == true) {
			model.addAttribute("message1", "面子がすべて順子ではない");
		} else if (p.isRiyu2() == true) {
			model.addAttribute("message2", "雀頭が役牌以外ではない");
		} else if (p.isRiyu3() == true) {
